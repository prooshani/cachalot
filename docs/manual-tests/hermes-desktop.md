...
Wait for `Uvicorn running on http://127.0.0.1:8011`. The line `prefix snapshots: N loaded` tells you whether a
...
minutes**, just inside Hermes's 300 s budget. The next main request then reuses the system block and
prefills only the rewritten history. If the summary times out, Hermes falls back to its deterministic
