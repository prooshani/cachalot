# Next-session prompt — **v30**, written 2026-09-22
...
## Job 3 — the quality gate has no Objective-C in it, and this agent can run it

Unchanged from v28 and v29, and the highest-value job that does **not** need a live session. Two live
coding turns have been put through a compiler and both fail on a model-level type error; the 40-case corpus
gate compiles C++ and parses Python, **contains no Objective-C at all**, and executes nothing. Add
Objective-C cases to `benchmarks/coding_tasks.json`, compile them in `code_validity.py` the way C++ is
compiled, and run what it builds on the cases with no input —
`docs/live-turns/2026-09-21-json2csv-2/` is the concrete case for why compiling alone is not the check:
repaired in one line it compiles, runs, exits 0, and still contradicts its own documented column order
because it builds a header from an unordered `allKeys`.
...
| `benchmarks/coding_quality.py --resume` | the 40-case corpus, restartable — **C++ and Python only, Job 3** | ~1.5 h |
