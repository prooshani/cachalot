# Next-session prompt — **v46**, written 2026-09-25
...
|---|---|---|---|
| **v46** | 2026-09-25 | Hamed asked for GLM-5.3-Flash on Cachalot next to DeepSeek | **0.17.0: GLM-5.3-Flash (Vontra MLX 4-bit) runs through `./serve-glm.sh` / `./chat-glm.sh` with experts streamed from the internal SSD: 3.3-3.6 tok/s decode, 14.3 all-resident, tools and thinking work. DeepSeek unchanged.** §17 |
...
**Read this first.** `docs/HANDOFF.md`'s "Start here (2026-09-25, 0.17.0)" block and section **17** (GLM), then
15.12.

**Hamed's standing priority order: Hermes usage first, vision second, speed/performance third.**

**Check the working tree before starting** (`git status --short`). If 0.17.0 is uncommitted, confirm with
...
## GLM jobs (new, §17). Ask Hamed which model is the priority before starting; if unsaid, GLM Job G1 then DeepSeek Job 1.

- **G1 — disk prefix snapshots for GLM.** Without them every server start prefills Hermes's ~20k-token system
  prompt at ~12.5 tok/s (~27 min). Reuse `snapshot_store.SnapshotStore`'s policy with a GLM cache serializer
  (every cache object's arrays by name, plus the object types), keyed on a GLM numerics version.
- **G2 — GLM routing trace, hotlist preload, one-layer-early prediction.** GLM is miss-bound (70 ms compute floor,
  ~225 ms of reads per token). Record routes for the Hermes dump's prompts, build a hotlist, and port the
  DeepSeek prediction (`CACHALOT_PREDICT_*`) to `StreamingSwitchGLU`. Measure with `chat-glm.sh` turns.
- **G3 — reply splice for GLM** (turn 2 reused 206 of 224: the template re-renders the tool call differently).
- **G4 — prefill speed** (12.5 tok/s on 248 tokens): profile whether the KDA gated-delta kernel or the per-expert
  loop dominates; batch the per-expert loop the way DeepSeek's prefill does.
- **G5 — quality gate**: a teacher-forced NLL/KLD check of the streamed path against mlx-vlm's stock SwitchGLU on
  a few layers (the streamed experts use per-expert `quantized_matmul`, the reference `gather_qmm`).
- Later: vision (mlx-vlm has the tower), MTP (native depth 1), a contiguous per-expert bank.
