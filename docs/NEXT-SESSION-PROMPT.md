# Next-session prompt — **v38**, written 2026-09-23
...
**Read this first.** `docs/HANDOFF.md`'s new "Start here (2026-09-23)" block, then sections **15.1**
...
**Check the working tree before starting.** The 0.10.0 work may still be uncommitted. `git status --short`.
If so, confirm with Hamed and commit with the release-standards bump already made (pyproject, `__init__`,
CHANGELOG and README are all at 0.10.0).
...
re-prefill, not one per turn.

## Job 2 — the first request of a server's life still costs 160-285 s. Two levers, both measured-first.

1. **Snapshot at the message boundary, not only at chunk multiples.** A new session reuses up to the last
   4,096 boundary inside the shared system prompt: 12,288 of 13,495 tokens, 1,207 re-prefilled, 18.9 s.
   Snapshotting where the system prompt ends would make that ~0 tokens. The engine knows the messages; the
   hard part is finding the token offset where the rendered system block ends. Check with the official
   encoding that the rendered system block is a token prefix of every request that shares it before relying
   on it. Size the win first (≤ ~15 s per new session).
2. **Persist the system-prompt snapshot to disk across server restarts.** Every `serve.sh` restart pays the
   full cold 13.5k-token prefill again on its first Hermes request. A snapshot is ~33 MB (§15.3). Writing
   the chunk-boundary snapshots of the longest reused prefix to disk, keyed by the token hash and the bank
   identity, and loading them at startup would take that to ~20 s. It must be invalidated by any change to
   the bank, the runtime version or the checkpoint.

Do not re-open chunk size as a speed lever without a clean A/B: §15.2's two A/Bs disagree (4096-chunking
was free in one and +17 % in the other), and same-process arms carry a cache-warmth order bias. If you
measure it, alternate arm order and use separate processes.

## Job 3 — vision, beyond the first check.
...
## Job 4 — batched prefill sits 0.014-0.02 mean KL from token-by-token decode. Size it.
...
## Job 5 — the 54 GiB collapse, re-tested with the right variable. Unchanged, low priority.
...
## Job 6 — score the banks on the Objective-C cases. Unchanged, not run.
...
- **`pgrep -f PATTERN` matches the shell that runs it when PATTERN is in its own command line.** Twice
  ...
  PID (`kill -0 $PID`), not on a pattern. New.
...
- **Arms of an A/B run in one process, in one order, share an expert cache that the earlier arms warm.**
  ...
  comparison only with alternated order or separate processes. New.
...
| `benchmarks/prefix_snapshot_exactness.py N...` | snapshot → restore bit-identity and snapshot size, new | ~5 min |
...
| `benchmarks/coding_quality.py` | the 52-case corpus, Job 6 | ~1.5-2 h |
...
| `src/cachalot/model/prefix_cache.py` | image-span identity in `find`, LRU on hit |
