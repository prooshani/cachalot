...
from v41runtime.model.moe_layer_metal import moe_layer_forward
...
def compressed_index_source_block_decode(
    layer_id: int,
    x: mx.array,
    *,
    start_pos: int,
    ...
    """
    Complete one-token DeepSeek V4.1 Flash backbone block for an
    INDEX-ONLY compressed-attention source layer.

    Initial released topology:
    ...
    Instead it consumes the latest values in SharedAttentionRuntime
    published by an earlier source layer.

    Ordering:

        HC attention coefficients
        -> hc_pre(previous FFN pre-mix)
        -> RMSNorm
        -> compressed attention reuse
        -> hc_post
        -> HC FFN coefficients
        -> hc_pre(attention pre-mix)
        -> RMSNorm
        -> MoE
        -> hc_post
    ...
    window_cache:
        This consumer layer's OWN raw/window KV cache.

        Compressed KV is not owned by this layer. It is read
        through shared_attn.compress_kv.

    shared_attn:
        Cross-layer source state. At minimum, a prior source layer
        must have published:
    ...
    updated_window_cache:
        This consumer layer's updated raw/window KV cache.
    ...
    #
    # As in the source/sliding-window blocks:
    #
    #   - hc_mixes sees the complete current residual stream.
    #   - attention collapses x using pre_mix received from the
    #     previous block.
    #   - attn_pre produced here is consumed by this block's FFN.
    # ========================================================
