...
| Third model: MiniMax-M3 (MLX 3-bit), experts streamed from SSD (`./serve-minimax.sh`, `./chat-minimax.sh`) | ✅ 0.22.0: text, tools, thinking, prefix cache in memory and on disk, expert cache kept across restarts; prefill ~240 tok/s at 16k (a 17k agent block in 101 s), 148 tok/s at 64k; decode 3.3 tok/s after a 2k prefill (4.1-4.7 on short chat turns); at 32k and 64k a token is 5 % and 8 % faster than in 0.21.0 on the same text; a second copy on the X10Pro adds read bandwidth |
