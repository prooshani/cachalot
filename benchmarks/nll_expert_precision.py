...
def main():
    ...
    with TextDecodeRuntime(MODEL_PATH, max_seq_len=4096) as rt:
        ...
        print(f"{args.experts}: tokens {len(nll)} | mean NLL {mean:.4f} nats | ppl {math.exp(mean):.3f} | "
        ...
        out = RESULTS_DIR / f"nll_experts_{args.experts}.json"
        out.write_text(json.dumps({"experts": args.experts, "prefill": args.prefill, "tokens": len(nll), "mean_nll": mean,
                                   "nll": nll, "argmax": argmax, "targets": ids[args.prefill:args.prefill + args.tokens]}))
        for other_mode in ("fp4", "oq3e", "runtime"):
            other = RESULTS_DIR / f"nll_experts_{other_mode}.json"
            if other_mode == args.experts or not other.exists():
                continue
